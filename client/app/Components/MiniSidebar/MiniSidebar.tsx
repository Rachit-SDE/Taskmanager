...
function MiniSidebar() {
  ...
  const getStrokeColor = (link: string) => {
    return pathname === link ? "#fed036" : "#71717a";
  };
  ...
  return (
    <div className="basis-[5rem] flex flex-col bg-background">
      <div className="flex items-center justify-center h-[5rem]">
        <Image src="/logo.png" width={60} height={60} alt="logo" className="ml-[6px]" />
      </div>
        ...
        <ul className="flex flex-col gap-10">
          {navItems.map((item, index) => (
            <li key={index} className="relative group">
              ...
              {/* Hover Tooltip */}
              <span className="u-triangle absolute top-[50%] translate-y-[-50%] left-8 text-xs pointer-events-none text-white bg-yellow-400 px-2 py-1 rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                {item.title}
              </span>
            </li>
          ))}
